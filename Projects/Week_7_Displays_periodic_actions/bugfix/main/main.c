...
// Длительность фазы вспышки светодиода (мс)
#define BLINK_MS 100
...
// Таймер отключения/управления вспышкой LED
static esp_timer_handle_t led_off_timer;

// Состояние автомата вспышки:
//   0 — LED горит (1-я фаза)  → выключить, запустить паузу
//   1 — LED выключен (пауза) → включить, запустить 2-ю фазу
//   2 — LED горит (2-я фаза)  → выключить, сброс
static volatile int blink_state = 0;

// Функция обратного вызова таймера — автомат двойной вспышки
static void led_off_timer_callback(void *arg)
{
    switch (blink_state) {
    case 0: // конец 1-й фазы горения
        gpio_set_level(LED_GPIO, 0);
        blink_state = 1;
        esp_timer_start_once(led_off_timer, BLINK_MS * 1000ULL); // пауза
        break;
    case 1: // конец паузы — начало 2-й вспышки
        gpio_set_level(LED_GPIO, 1);
        blink_state = 2;
        esp_timer_start_once(led_off_timer, BLINK_MS * 1000ULL); // 2-я фаза
        break;
    case 2: // конец 2-й фазы горения
        gpio_set_level(LED_GPIO, 0);
        blink_state = 0;
        break;
    default:
...
static void buttons_task(void *arg)
{
  ...
  // Конфигурируем и создаём таймер вспышки
  const esp_timer_create_args_t led_off_timer_args = {
      .callback = &led_off_timer_callback,
      .name = "led_off",
  };
  esp_timer_create(&led_off_timer_args, &led_off_timer);
  ...
  while (1)
  {
    ...
    if (evt.type == BTN_EVT_PRESS)
    {
      ...
      if (!state->is_pressed)
      {
        ...
        {
          // Двойная вспышка светодиодом через таймер (неблокирующая)
          printf("BTN3 action: blink LED (double, %d ms)\n", BLINK_MS);
          esp_timer_stop(led_off_timer); // сброс, если предыдущий цикл ещё не завершился
          blink_state = 0;
          gpio_set_level(LED_GPIO, 1);
          esp_timer_start_once(led_off_timer, BLINK_MS * 1000ULL);
        }
